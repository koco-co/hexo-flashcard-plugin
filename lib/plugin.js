...
const MARKDOWN_CODE_BLOCK = /<pre><code class="(?:lang|language)-([^"\s]+)">([\s\S]*?)<\/code><\/pre>/g;
...
function registerPlugin(hexo) {
  ...
  const renderMarkdown = (text) => {
    const html = hexo.render.renderSync({ text, engine: 'markdown' });
    const highlighterName = hexo.config.syntax_highlighter;
    return html.replace(MARKDOWN_CODE_BLOCK, (source, lang, encodedCode) => {
      if (/<span\b/i.test(encodedCode)) return source;
      const code = decodeCodeEntities(encodedCode).replace(/\n$/, '');
      try {
        const highlighted = hexo.extend.highlight.exec(highlighterName, {
          context: hexo,
          args: [code, { lang, lines_length: code.split('\n').length }]
        });
        return highlighted.replace(/{/g, '&#123;').replace(/}/g, '&#125;');
      } catch {
        return source;
      }
    });
  ...
  hexo.extend.generator.register('flashcard-learning', function flashcardGenerator() {
    ...
    const routes = [
      ...
      { path: `${current.assetPath}/flashcard-queue.js`, data: () => fs.createReadStream(path.join(pluginRoot, 'assets', 'flashcard-queue.js')) },
  ...
  hexo.extend.injector.register('body_end', () => {
    ...
    const syncScripts = current.sync.enabled
      ? `<script id="hfc-sync-config-data" type="application/json">${jsonResource(current.sync)}</script><script defer src="${assetBase()}/supabase.js"></script><script defer src="${assetBase()}/flashcard-sync.js"></script>`
      : '';
    return `<script defer src="${assetBase()}/ts-fsrs.umd.js"></script><script defer src="${assetBase()}/flashcard-queue.js"></script>${syncScripts}<script defer src="${assetBase()}/flashcard.js"></script>`;
  }, 'default');
}
