...
function renderLearningPage({ root, assetPath, learningPath, sync = { enabled: false } }) {
  const assetBase = joinUrl(root, assetPath);
  const syncPanel = sync.enabled ? `
  <section class="hfc-sync" data-hfc-sync data-hfc-sync-state="local" aria-label="复习进度同步">
    <div class="hfc-sync__copy"><strong>复习进度</strong><span data-hfc-sync-status>未登录，进度仅保存在此浏览器</span></div>
    <div class="hfc-sync__actions">
      <button class="hfc-sync__button hfc-sync__button--primary" type="button" data-hfc-login>使用 GitHub 登录并同步</button>
      <button class="hfc-sync__button" type="button" data-hfc-sync-now hidden>立即同步</button>
      <button class="hfc-sync__button" type="button" data-hfc-logout hidden>退出登录</button>
    </div>
  </section>` : '';
  return `
