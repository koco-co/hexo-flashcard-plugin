...
| 练习模式 | 今日练习、新卡练习、随机练习 | 今日练习使用当天任务；新卡按高频未复习优先且每天最多首次开始 50 张；随机练习独立运行 |
| 复习筛选 | `article`、`deck`、`tag` 或 `priority` 查询参数 | 先限定范围，再分别建立三种练习队列；计划数量与今日练习队列保持一致 |
| 本地调度 | 忘记、模糊、记得、简单 | 由 `ts-fsrs` 计算下一次到期时间 |
| 复习计划 | 已保存的 FSRS 结果 | 月历显示每日数量与打卡状态，点击日期后每批加载 20 题 |
| 可选云同步 | GitHub 登录与 Supabase 配置 | 本地先保存，后台按最新评价合并卡片、每日历史和新卡开始记录，并保持跨设备每日上限 |
...
三段内容都支持标准 Markdown 围栏代码。提供语言名称后，文章卡和复习卡会显示同一套深色代码块、行号、语言名称、三色状态点、复制按钮和折叠按钮：
...
- 代码块默认展开；点击箭头只折叠代码正文，工具栏仍然保留。
- 复制只包含源代码，不包含行号和语言名称，并显示“已复制”或“复制失败”。
- 点击、选择或横向滚动代码不会触发闪卡翻面；长代码不会撑破卡片宽度。
- 工具栏由插件自身初始化，兼容动态复习卡和 PJAX，不依赖 Butterfly 主题脚本。
...
2. 页面提供“今日练习（剩余 N）”“新卡练习（剩余 N）”和“随机练习”三个独立入口；今日练习使用当天任务，新卡练习按高频未复习优先，每个 Asia/Shanghai 自然日最多首次开始 50 张。
3. 当前范围内当天任务中尚未完成的卡片形成不可回退的今日练习快照；月历当天的题数与今日练习队列保持一致。
4. 读者翻面后选择忘记、模糊、记得或简单。
5. 插件用相同时间点和相同参数提交按钮所预览的 <b>FSRS</b> 结果，然后自动进入下一张；只有今日练习评价影响每日打卡，新卡练习和随机练习不影响打卡。
6. 月历汇总已复习卡片的 `due` 与每日任务，主页显示日期、数量和状态；点击日期后，每批最多读取并渲染 20 道问题。每次评价后立即重排，未评价的新卡不进入计划。
7. 当天到期任务全部完成后，日期显示绿色渐变圆环与勾；当天或过去仍有未完成任务时显示红色渐变圆环。次日补做不会改写前一天的红色历史。

浏览器进度、每日任务快照、打卡历史和新卡首次开始记录使用稳定卡片 `id` 保存在 `localStorage`。新卡额度按 Asia/Shanghai 自然日重置；未登录时不同浏览器或设备互不继承，登录同一 <b>GitHub</b> 账号后由已配置的 <b>Supabase</b> 项目合并并执行跨设备上限。产品不提供补签；未登录清除本地进度，登录后全局重置需要输入固定确认值。
