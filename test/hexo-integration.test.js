...
test('generates article previews, CTA, learning page, and assets in a real Hexo site', async (t) => {
  ...
  const queueJs = fs.readFileSync(path.join(siteRoot, 'public', 'flashcard-assets', 'flashcard-queue.js'), 'utf8');
  ...
  assert.match(postHtml, /flashcard-assets\/flashcard-queue\.js/);
  ...
  assert.match(postHtml, /id="hfc-sync-config-data"/);
  assert.match(postHtml, /"enabled":true/);
  ...
  assert.match(postHtml, /<figure class="highlight sql">/);
  assert.match(postHtml, /<td class="gutter">/);
  assert.match(postHtml, /<span class="keyword">SELECT<\/span>/);
  assert.match(postHtml, /SELECT/);
  assert.match(postHtml, /FROM/);
  ...
  assert.match(learningHtml, /flashcard-assets\/flashcard-queue\.js/);
  ...
  assert.deepEqual(cardIndex.find((card) => card.id === 'http-404').articles.map((article) => article.articlePath).sort(), ['demo/', 'reference/']);
  ...
  const codeCard = shardCards.find((card) => card.id === 'http-404');
  assert.match(codeCard.explanationHtml, /<figure class="highlight sql">/);
  assert.match(codeCard.explanationHtml, /<td class="gutter">/);
  assert.match(codeCard.explanationHtml, /<span class="keyword">SELECT<\/span>/);
  ...
  assert.match(css, /\.hfc-code-toolbar\s*{/);
  assert.match(css, /\.hfc-code-copy\[data-hfc-code-copy-state='success'\]/);
  assert.match(css, /\.hfc-code--collapsed > table/);
  assert.match(css, /\.hfc-code td\.gutter/);
  ...
  assert.match(js, /今日练习（剩余 \$\{todayRemaining\}）/);
  assert.match(js, /新卡练习（剩余 \$\{newRemaining\}）/);
  assert.match(js, /label: '随机练习'/);
  assert.doesNotMatch(js, /开始学习新卡/);
  assert.doesNotMatch(js, /暂无到期卡片/);
  ...
  assert.match(js, /function scopeCards/);
  assert.match(js, /function todayTaskSummary/);
  assert.match(js, /function todayTaskEntries/);
  assert.match(js, /const scopedTasks = entriesForDate\(localDateKey\(Date\.now\(\)\)\)/);
  assert.match(js, /return entriesForDate\(localDateKey\(Date\.now\(\)\)\)/);
  assert.match(js, /\.filter\(\(\[id, task\]\) => scopedIds\.has\(id\) && Number\.isFinite\(task\?\.due\)\)/);
  assert.match(js, /localDateKey\(entry\.due\) === key && !isNewLearningCard\(progress\.cards\[entry\.card\.id\]\)/);
  assert.match(js, /function planEntriesForDate/);
  assert.match(js, /key === localDateKey\(Date\.now\(\)\)[\s\S]*?entries\.filter\(\(\{ completedAt \}\) => !Number\.isFinite\(completedAt\)\)/);
  assert.match(js, /const entries = planEntriesForDate\(key\)/);
  assert.match(js, /const entries = entriesForDate\(key\)/);
  assert.match(js, /function randomCards/);
  assert.match(js, /left\.due - right\.due/);
  assert.match(js, /data-hfc-mode="\$\{button\.mode\}"/);
  assert.match(js, /button\.dataset\.hfcMode === 'random'/);
  assert.match(js, /随机练习/);
  assert.doesNotMatch(js, /随机复习/);
  assert.match(queueJs, /NEW_CARD_DAILY_LIMIT = 50/);
  assert.match(queueJs, /TIME_ZONE = 'Asia\/Shanghai'/);
  assert.match(queueJs, /compareFreshCards/);
  assert.match(queueJs, /quotaRemaining/);
  assert.match(js, /function loadCardContent/);
  assert.match(js, /function renderCalendar/);
  assert.match(js, /function openDrawer/);
  assert.match(js, /function prepareCodeBlocks/);
  assert.match(js, /function copyCode/);
  assert.match(js, /navigator\.clipboard\?\.writeText/);
  assert.match(js, /data-hfc-code-collapse/);
  assert.match(js, /\[data-hfc-code\]/);
